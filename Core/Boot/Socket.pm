...
method handleListen() {
		...
		foreach (@arrSocks) {
			...
			eval {
				...
				if ($intBytes == 0) {
					$self->{child}->{mysql}->updateOnlineStatus(0, $objClient->{penguin}->{ID});
		...
		$self->runCrons() unless ($self->{child}->{config}->{nitro}->{type}->{value} eq 'login');
}

method runCrons() {
		my $intTime = time;
		my $intTimer = 10;
		my $intStamp = $intTime + $intTimer;
		if (!$self->{jobStamp}) {
			$self->{jobStamp} = $intStamp;
		} elsif ($intTime > $self->{jobStamp}) {
			$self->handleUpdateServerPopulation;
			$self->{jobStamp} = $intStamp;
		}
}
...
method handleAddClient() {
		...
		my $objClient = Gas->new($self->{child}, $resSocket);
		$self->{clients}->{$intKey} = $objClient;
}

method handleUpdateServerPopulation() {
		my $strName = $self->{child}->{config}->{nitro}->{server_name}->{value};
		my $intPort = $self->{child}->{config}->{nitro}->{port}->{value};
		my $intPop = scalar(keys %{$self->{clients}});
		$self->{child}->{mysql}->updatePopulation($intPop, $intPort);
		$self->{child}->{logger}->info('Server: ' . $strName . '|Population: ' . $intPop);
}
