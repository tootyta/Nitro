...
method connectMySQL($strHost, $strUsername, $strPassword, $strDatabase) {
		my $connection = Mojo::mysql->new('mysql://' . $strUsername . ':' . $strPassword . '@' . $strHost . '/' . $strDatabase) or die($self->{child}->{logger}->error('Failed to Connect to MySQL Server'));		
		$self->{connection} = $connection;
		$self->{child}->{logger}->info('Successfully connected to MySQL server');
}

method createGameServer($intPort, $strName, $strIP) {
		my $arrResults = $self->{connection}->db->query('select count(*) from servers where port = ?', $intPort);
		while (my $arrData = $arrResults->hash) {
			if ($arrData->{"count(*)"} <= 0) {
				$self->{connection}->db->query('insert into servers (port, name, ipaddress) values (?, ?, ?)', $intPort, $strName, $strIP);
			}
		}
}
...
method generateServerList {
		my $string  = $self->{connection}->db->query("select * from servers where type='game'")->hashes->map(sub {
									my $intBars = round(($_->{population} / $_->{max_pop}) * 5);
									$_->{ipaddress} . ':' . $_->{port} . ':' . $_->{name} . ':' . $intBars 
		})->join('|');
		return $string;
}	

method getServerLimit($intPort) {
		my $arrResults = $self->{connection}->db->query('select max_pop from servers where port = ?', $intPort);
		while (my $arrData = $arrResults->hash) {
				return $arrData->{max_pop};
		}
}

method getBuddiesOnline($strName, $objClient) {
	my $arrResults = $self->{connection}->db->query('select buddies from kittens where username = ?', $strName);
	while (my $arrData = $arrResults->hash) {
		my @arrBuddies = split(',', $arrData->{buddies});
		foreach my $strBuddy (@arrBuddies) {
			my ($intBuddyID, $strBuddyName) = split('\\|', $strBuddy);
			return $self->getOnlineStatus($intBuddyID);
		}
	}
}

method updateOnlineStatus($blnStatus, $intPengID) {
	   $self->{connection}->db->query('update kittens set estatus = ? where ID = ?', $blnStatus, $intPengID);
}

method getOnlineStatus($intPengID) {
		my $arrResults = $self->{connection}->db->query('select estatus from kittens where ID = ?', $intPengID);
		while (my $arrData = $arrResults->hash) {
				return $arrData->{estatus};
		}
}
...
method updatePopulation($intPop, $intPort) {
		$self->{connection}->db->query('update servers set population = ? where port = ?', $intPop, $intPort);
}
