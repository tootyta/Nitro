...
method leaveTable($objClient) {
		...
		if ($tableId ne 0) {
			my $seatId = first_index { $_ eq $objClient} @{$self->{child}->{gamesys}->{systems}->{Tables}->{playersByTableId}->[$tableId]};
...
method handleStartGame($strData, $objClient) {
		my $tableId = $objClient->{gaming}->{tableID};
		if ($tableId ne 0) {
			my $seatId = scalar(keys @{$self->{child}->{gamesys}->{systems}->{Tables}->{tablePopulationById}->[$tableId]}) - 1;			
			$objClient->sendData('%xt%jz%-1%' . $seatId . '%');
			if($seatId < 2) {
				$objClient->sendRoom('%xt%uz%-1%' . $seatId . '%' . $objClient->{penguin}->{username} . '%');
				if($seatId == 1) {
					foreach my $objPlayer (values @{$self->{child}->{gamesys}->{systems}->{Tables}->{playersByTableId}->[$tableId]}) {
				...
				if ($isPlayer < 2 && $gameReady >= 2) {
					if ($self->{child}->{gamesys}->{systems}->{Tables}->isValidFourTable($tableId)) {
						...
						if (looks_like_number($chipColumn) && looks_like_number($chipRow)) {
							my $seatId = first_index { $_ eq $objClient} @{$self->{child}->{gamesys}->{systems}->{Tables}->{tablePopulationById}->[$tableId]};
							my $libID = $seatId + 1;
